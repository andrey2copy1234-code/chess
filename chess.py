try:
    ...
    class game():
        __slots__ = ('root','map','mode','Frames','step','label','canvas','pictures','highlighted','file','victory','steps','hist','histPos','speed','text','mode', 'modes', 'snow_analiz_steps', 'on_click_fns', 'displaying_ntfct', 'saving_in')
        ...
        def start(self):
            if mode == "play" or mode == "random" or mode[0] == "b" or mode == 'minimax':
                    ...
                    if not self.victory and (self.mode != "random"):
                        ...
                        for ob in self.Frames:
                            if ob.pos == pos:
                                ...
                                if self.saving_in:
                                    try:
                                        with open(self.saving_in, 'r', encoding='utf-8') as f:
                                            pre = f.read()
                                    except:
                                        pre = ""
                                    with open(self.saving_in, 'w', encoding='utf-8') as f:
                                        f.write(pre+f"{alphabet[int(self.highlighted.pos[0])-1]}{int(self.highlighted.pos[1])}-{alphabet[int(ob.pos[0])-1]}{int(ob.pos[1])}\n")
                ...
                def Control_alt_s(key):
                    self.saving_in = simpledialog.askstring("Сохранение", "Куда хотите сохранить")
                self.root.bind("<Control-Alt-s>",Control_alt_s)
                ...
                def give_path(ob, step):
                    ...
                    if l_steps == 0 or l_steps == 1:
                        ...
                        return res
                    return 1
                def is_checkmate(ob, step):
                    step_vec = v.Vector2(mas=step)
                    enemy_color = not ob.b
                    
                    # 1. Симулируем наш ход
                    with imagine_step(ob, step_vec):
                        # 2. Проверяем, находится ли король врага под шахом после хода
                        enemy_king = get_king(enemy_color)
                        if enemy_king is None:
                            return True # Короля нет — технический мат
                            
                        if not attak(enemy_king):
                            return False # Если шаха нет, то это не мат

                        # 3. Ищем, есть ли у врага ХОТЯ БЫ ОДИН легальный ответ
                        for enemy_fig in self.map:
                            if enemy_fig.b == enemy_color:
                                for move in get_steps(enemy_fig):
                                    move_vec = v.Vector2(mas=move)
                                    # Используем вашу "умную" can_attak_king
                                    # Она проверит, спасет ли этот ход короля
                                    if can_attak_king(enemy_fig, move_vec):
                                        return False # Враг может спастись, мата нет

                        return True # Шах есть, а легальных ходов у врага нет — МАТ
                ...
                def eval_step(ob, step):
                    return (1*
                        ...
                        (10000 if is_checkmate(ob, step) else 1))
    ...
    def load_modes(folder='./modes'):
        try:
            ...
            try:
                ...
                messagebox.askyesno("Нет прав", "Попробуйте запустить программу от имени администратора или проверить незапушенна ли она")
                return []
