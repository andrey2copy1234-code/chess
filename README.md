# Chess Project 🏁

Проект шахмат с поддержкой игры против ИИ, сохранениями, справкой, сохранениями видео и многим другим.

## 🧩 Структура проекта

### Основные модули:
- `chess.py` - Главный исполняемый файл
- `vector.py` - Библиотека векторных операций
- `chess_module.py` - Модуль игровой логики

## 📚 Детальное описание

### 1.1 Архитектура проекта
Проект разделён на три основных компонента:
- **chess** - Ядро приложения
- **vector** - Математическая библиотека
- **chess_module** - Модуль игровой механики

### 1.2 Модуль Vector (vector.py)
Библиотека векторных операций, написанная на чистом Python с использованием модуля array.

#### Основные классы:
| Класс | Назначение |
|---------------------|----------------------------------------------------------------------------|
| `binInt` | Работа с большими целыми числами (до 2^64 бит). Поддерживает только положительные числа и ноль. |
| `binIntPAM` | Нерабочий класс. |
| `VF` | Базовый класс для всех векторных типов. с помошью его ещё можно определить, является ли аргумент вектором с помощью `VF in item.__mro__`. |
| `Vector` | Универсальная векторная коллекция. |
| `Vector2` | 2D-вектор c ограниченной длиной в виде двух элементов. |
| `Vector2AndPos2` | Контейнер для пары векторов (позиция + направление). |

#### Пример использования binInt:
```python
num = binInt(100)
num += 1
print(num) # 101
num *= 5
print(num) # 505
```

#### Функциональность:
1. **Преобразования:**
   - `fromDegToRad()` - Конвертация градусов в радианы
   - `fromRadToDeg()` - Конвертация радианов в градусы
   - `to_vector()` - Универсальный конвертер в вектор. Можно задать правило преобразования с помощью метода `__vector__`.

2. **Генерация:**
   - `CreateMatrix2Rotate()` - Матрица вращения из угла
   - `vrotate()` - Направляющий вектор из угла
   - `vrandom` - Подмодуль генерации случайных значений:
     - `vrandom.vrandom` - Вектор с рандомными значениями от start до end длиной l
     - `vrandom.vrandomitem` - Несколько рандомных элементов в векторе длиной l (аналог getRandomItem)
     - `vrandom.vrandomProcentitem` - Несколько рандомных элементов в векторе длиной l (аналог getRandomProcentItem)

3. **Операции:**
   - `GetRandomItem()` - Случайный элемент коллекции
   - `GetRandomProcentItem()` - Вероятностный выбор (принимает коллекцию `((procent, item),...)` или словарь `{procent: item,...}`)
   - `vrange()` - Векторизированный аналог range (не генератор, практически бесполезен)
   - `vfromline()` - Создание вектора из кода python

### 1.3 Модуль Chess (chess_module.py)
Ядро шахматной логики с графическим интерфейсом.

#### Основные компоненты модуля:

##### Классы:
* **Tween** - Класс для создания анимаций
* **figure** - Базовый класс для шахматных фигур
* **histStr** - Класс для работы с историей ходов
* **LoadedError** - Исключение для ошибок загрузки
* **NaF_Error** - Исключение для ошибок "Not a File"

##### Основные функции:
* **multiple_choice** - Создание диалоговых окон с множественным выбором. работает на множестые os
* **FCinP** и **FPinC** - Преобразование координат из клеточек в пиксили и из пиксилей в клеточки
* **change_dirictory_images** - Смена директории изображений. **работает только в начале** запуска chess_module а потом не работает
* **check** - Проверка настроек и задование их
* **check_images** - Проверка наличия изображений
* **loaded** и **loadedString** - Загрузка игры из файла или строки
* **save** и **saveV** - Сохранение игры. save это в файл а saveV это сохранение видео.
* **openV** - загрузка видео из файла
* **setSize** - Установка размера поля. также можно через `chess_module.size = new_size`

##### Глобальные данные:
* **SETUP_DEFAULT** - Настройки по умолчанию
* **figure_moves** - то насколько может сходить фигура от сейчашнего положения. значением может быть и tuple и функция принимающяя фигуру и функцию столкновения.
* **figure_picture** - Пути к изображениям фигур
* **size** - Размер игрового поля в пикселях

### 1.4 Главный модуль Сhess
Данный модуль контролирует вызывает и даёт команды на отрисовки всему остальному.
Именно Chess контролирует режимы игры, создание окна, съедание фигур, создание поля, бота. Это уже сложно объяснить только лишь функциями и т.д. поэтому объясню логикой и функциями.
При старте первое что происходит это скрытие консоли. дольше нам даётся выбор какой режим мы хотим запустить. после этого нас спрашивают хотим ли мы загрузить файл. и в одном и в другом случае при создании игры передатся при создании game аргумент text. этот аргумент просто отвечает за обычную карту. но потом вызывается `__init__` в нём происходит загрузка картинок и создания шаблона для окна. шаблон таков. на всё растянут холст для рисования. мы на нём будем рисовать буквально всё.
Потом вызывается метод start. он сразу рисует клеточки. потом рисует (используя метод init у фигуры) на экране все фигуры. после этого всего последним рисуются (это не виджеты) кнопочки. и появлятся множество помагаюших функций (clickReal, open_file, restart, get_steps и другие). clickReal можно разделить на три облости. первоя это нажатия на квадратики. вторая нажатия на фигуры. третия это нажатия (ему не важно сколько в низ нужного места) на кнопочки.
Потом мы можем увидить создания функций которые привязавуются к клавишам (Control-z, Control-Shift-Z и другие). потом мы можем увидить комент `#circle` этот комент означает что всё что дальше это функции для бота и второй цикл для всех режимов (кроме play). каждая функция подписана комментом что-бы вам легче с ней было разобраться. но размеру как они (названия функций) строятся. сначало если мы смотрим после передвежения то в начало добавляем move а дальше can обозначает можно, atack и attack обозначает атака этой фигуры (или взятие если раньше был can), protection - означает застчита (может означать что фигура образует такую застную цепочку или просто  потом окажется под застчитой). и всё это через '_'.

#### Работа бота
Бот как и большендство режимов работает во втором цикле.
бот думает по правилам. их можно увидеть в их подстчёте.

бот отличается от бот2 только тем что одно работает рандомом а второе балами

### 1.5 То как делать моды
для создания мода нам понядобится создать файл .py. а в нём класс main:
```python
class main():
   pass
```
у main есть обязательные атрибуты:
- name - имя мода
- description - описание работы мода
а также есть обязательные методы:
- `init(game)` - даю game вы его куда-нибудь сохраните
- `start()` - вызывается при выборе мода
- `click(pos)` - срабатывает при клике
- `iteration(mode)` - срабатывает каждый тик (так бот например работает). исключения не ловятся.

### 1.6 Основные возможности игры

#### Управление игрой:
* **Режимы игры**:
  * play - обычный режим
  * random - случайная игра (рандом против рандома короче)
  * bot - игра с ботом (думает рандомом но понимает что лучше а что хуже)
  * bot2 - улучшенный бот (думает баллами сразу самые лучшие проверяет)
  * bot2 vs bot2 - игра ботов между собой
  * bot_random - случайный бот (полный рандом)

#### Горячие клавиши:
* **Control-h** - Справка
* **Control-z** - Отмена хода
* **Control-Shift-Z** - Возврат отмены
* **m** - Смена режима
* **c** - Очистка истории
* **s** - Изменение скорости главного цикла (полезно для тестирования)
## Часть Вопрос - Ответ
* Что такое ArrayC? - ArrayC это библеотека типа numpy. можно посмотреть то что она умеет посмотрев её тесты. пока-что библеотека только тестируется и может быть нестабильной в каких-то случаях.
* На чём написан ArrayC? - ArrayC написан на C++ в котором я делал C-обёртку и в python я тоже делал обёртку. итог - скорость NumPy.
* Почему ArrayC а не NumPy? - мне не понравился numpy. для того что бы что-то сделать в NumPy надо писать слишком много кода. поэтому я создаю ArrayC.
* Я полностью сам писал ArrayC? - да, это полностью моя библеотека.
* Я взял готовое решение с vector или сам создал? - сам создал.
* У меня есть мод. как его запустить в игре? - поместите в папку modes и перезапустите игру.

## 🚀 Особенности
- Поддержка кастомных векторных операций
- Гибкая система диалоговых окон
- Расширяемая архитектура
- Удобные но только для вычисления очень больших чисел

## ⚙️ Требования
- Python 3.8+ (точно неизвестно но как я смотрел вроде как на 3.6+ запустится)
- tkinter (для GUI)
- потом возможно numpy (но скорее всего ArrayC)

## 📌 В планах
- Реализация бинарных операций для binIntPAM
- Оптимизация векторных преобразований (уже происходит. переход с vector на ArrayC)
- Добавление сетевой игры (уже есть но отдельно)
- улучшение бота
- Облегчение создания модов
- и может быть что нибудь ещё будет
